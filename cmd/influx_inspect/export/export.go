...
func (cmd *Command) Run(args ...string) error {
   ...
   fs.BoolVar(&cmd.eol0, "eol0", false, "Use zero-byte as line terminator")
         ...
         if _, ok := cmd.walFiles[key]; ok {
            ...
            for _, wFile := range files {
               if strings.Contains(wFile, fmt.Sprintf("/%s/", shard)) {
                  fmt.Fprintf(msgOut, "exporting wal (%s) data for shard %s (%d of %d)...\n", wFile, shard, i+1, len(cmd.shards))
